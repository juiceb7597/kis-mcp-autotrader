...
class ApiExecutor:
    ...
    def _extract_trenv_params_from_example(cls, api_code_content: str) -> Dict[str, str]:
        ...
        for param_name, trenv_attr in matches:
            ...
            # 🎯 API 가이드가 대문자라 하더라도, 파이썬 함수 인자명은 소문자일 확률이 높습니다.
            # 일단 소문자로 표준화하여 저장합니다.
            dynamic_mappings[param_name.lower()] = trenv_value
    ...
    def _modify_api_code(cls, api_code_path: str, params: Dict[str, Any], api_type: str) -> str:
        ...
        try:
            ...
            # -------------------------------------------------------------
            # 👇 에러 유발 파라미터 모조리 강제 삭제
            # keys_to_remove = [
            #     "CANO", "cano", "ACNT_PRDT_CD", "acnt_prdt_cd",  # 계좌번호 관련
            #     "stock_name", "_original_search_value", "_resolved_stock_code", # 검색 꼬리표
            #     "ctx_area_nk100", "ctx_area_fk100", "tr_cont"    # 👈 [추가됨] 연속조회/페이징 꼬리표
            # ]
            # for k in keys_to_remove:
            #     adjusted_params.pop(k, None)
            # -------------------------------------------------------------
            ...
            # 2. 정화 및 강제 할당 로직
            for param_name, correct_value in account_mappings.items():
                # 함수 정의에 해당 파라미터가 있는지 확인 (소문자 기준)
                if param_name.lower() in function_params.lower():
                    
                    # [중요] 기존에 들어있던 대문자/소문자 중복 키들을 싹 다 지웁니다.
                    # list()로 감싸는 이유는 반복문 도중 dict 크기 변경 에러를 방지하기 위함입니다.
                    keys_to_remove = [k for k in adjusted_params.keys() if k.lower() == param_name.lower()]
                    for k in keys_to_remove:
                        del adjusted_params[k]
                        
                    # 정확한 소문자 키로 시스템 변수를 주입합니다.
                    adjusted_params[param_name.lower()] = correct_value
                    print(f"[보안강제] {param_name.lower()} 변수 교체 완료")
