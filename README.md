...
### ⚡ 3. 실시간 시장 데이터 연동 (MCP)
* **실시간 의사결정**: LLM이 정적인 지식에 의존하지 않고, **Model Context Protocol(MCP)**을 통해 한국투자증권의 실시간 호가, 잔고, 체결 내역에 직접 접근하여 가장 정확한 데이터를 바탕으로 판단합니다.
...
3. **PDF 추출**: **LLM(Gemini)**이 리서치 원문 PDF 다운로드 링크를 분석합니다.
4. **임베딩 및 저장**: **RagWire**를 통해 중복을 제거하고 **Qdrant (Vector DB)**에 저장합니다.
...
<details>
<summary>실제 발송되는 보고서 내용 보기 (클릭)</summary>
